const { withProjectBuildGradle, withXcodeProject, withPodfileProperties } = require('@expo/config-plugins');
...
function includeNativeModulePod(config) {
  return withPodfileProperties(config, async (podConfig) => {
    ...
    if (
      ...
      podConfig.modResults.pod('cbl-reactnative', { path: podspecPath });
    }
    return podConfig;
  });
}
